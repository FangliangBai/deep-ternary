...
class Autoencoder():
    ...
    def build_sparse_binary_sensing_matrix(self, inp, inp_dim, out_dim, scope):
        with tf.variable_scope(scope) as scp:
            ...
            scale = tf.get_variable('%s_binary_weight_scale' % scope,
                                    initializer=np.ones((1, self.y_dim)).astype(np.float32), dtype=tf.float32,
                                    trainable=False)
    ...
    def calc_spars_mask(self, tensor, spars_level):
        ...
        full_indices = tf.concat([tf.expand_dims(range_repeated, 2), tf.expand_dims(indices, 2)],
                                 2)  # change shapes to [N, k, 1] and [N, k, 1], to concatenate into [N, k, 2]
        full_indices = tf.reshape(full_indices, [-1, 2])

        mask = tf.sparse_to_dense(full_indices, tensor.get_shape(), sparse_values=1., default_value=0.,
    ...
    def partial_fit(self, x, lr, get_summary=False):
        ...
        if get_summary:
            loss, train_opt, summary = self.sess.run([self.recon_loss, self.train_opt, self.merged_summaries],
                                                     feed_dict=fd)
        else:
    ...
    def bn_layer(self, inputs, scope):
        bn = tf.contrib.layers.batch_norm(inputs, is_training=self.is_training,
                                          center=True, fused=False, scale=True, updates_collections=None, decay=0.9,
                                          scope=scope)
        return bn
